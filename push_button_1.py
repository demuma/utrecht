...
class HydraulicArmControl(Node):
    def __init__(self):
        ...
        self.gamma_sub = self.create_subscription(
            Float32, '/telemetry/gamma', self.gamma_callback, 10)
        self.beta1_sub = self.create_subscription(
            Float32, '/telemetry/beta_1', self.beta1_callback, 10)
        self.beta2_sub = self.create_subscription(
            Float32, '/telemetry/beta_2', self.beta2_callback, 10)
        self.beta3_sub = self.create_subscription(
            Float32, '/telemetry/beta_3', self.beta3_callback, 10)
        self.button_sub = self.create_subscription(
            Int8, '/telemetry/buttons', self.button_callback, 10)
        self.temperature_sub = self.create_subscription(
            Float32, '/telemetry/temperature', self.temperature_callback, 10)

        # --- Publishers ---
        self.servo_1_pub = self.create_publisher(Float32, '/robotic_arm/hydraulic_servo_1', 10)
        self.servo_2_pub = self.create_publisher(Float32, '/robotic_arm/hydraulic_servo_2', 10)
        self.servo_3_pub = self.create_publisher(Float32, '/robotic_arm/hydraulic_servo_3', 10)
        self.servo_4_pub = self.create_publisher(Float32, '/robotic_arm/hydraulic_servo_4', 10)
        
        self.pump_pub = self.create_publisher(Float32, '/robotic_arm/hydraulic_pump_throttle', 10)
